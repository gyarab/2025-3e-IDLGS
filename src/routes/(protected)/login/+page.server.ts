...
export const actions = {
	login: async (event) => {
		return await formRunner(event, ['email', 'password'], false, async (data: FormDataType, user: UserTypeFull | undefined) => {
			const userData = (await event.locals.db.select({
			...
			if (verifyPassword(data.password, userData.salt, userData.iterations, userData.password)) {
				...
				await event.locals.db.insert(databaseSchema.session)
				.values({
					userId: userData.id,
					token: sessionToken,
				});
	...
	logout: async (event) => {
		return await formRunner(event, [], true, async (data: FormDataType, user: UserTypeFull | undefined) => {
			const sessionToken = event.cookies.get("sessionTokenIDLGS") as string;

			await event.locals.db.delete(databaseSchema.session)
				.where(
	...
	logoutAll: async (event) => {
		return await formRunner(event, [], true, async (data: FormDataType, user: UserTypeFull | undefined) => {
			await event.locals.db.delete(databaseSchema.session)
				.where(
					eq(databaseSchema.session.userId, user!.id)
				);
