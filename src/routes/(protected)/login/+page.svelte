...
>
    <i class="ri-login-box-line absolute right-1/15 bottom-1/6 scale-200 rotate-15 text-9xl text-white opacity-20"></i>
    <i class="ri-lock-password-line absolute top-1/4 left-1/15 scale-200 -rotate-15 text-9xl text-white opacity-20"></i>

    <div
        class="z-10 flex w-full max-w-md flex-col gap-4 sm:gap-6 rounded-3xl p-4 sm:p-8 shadow-2xl backdrop-blur-sm sm:rounded-4xl {data.darkMode
            ? 'bg-neutral-800/80 text-white'
            : 'bg-white/90 text-black'}"
    >
        <header class="flex flex-col items-center gap-1 text-center pt-2">
			<img 
			...
			/>
            <div>
                <h1 class="text-2xl font-bold tracking-tight sm:text-3xl">{m.login()}</h1>
            </div>
        </header>
