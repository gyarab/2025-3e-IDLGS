...
export const actions = {
	updatePersonalInfo: async (event) => {
		return await formRunner(['degree', 'name', 'surname', 'day', 'month', 'year'], async (
			...
			) || !Number.isInteger(parseInt(formData['background'])) || parseInt(formData['background']) < 0) {
				return fail(400);
			}
	...
	deleteAccount: async (event) => {
		return await formRunner([], async (
			...
			try {
				await event.locals.db.delete(schema.user)
					.where(eq(schema.user.uuid, user.uuid));
			} catch (e) {
				writeLog(event, 'ERROR', 'DB failure.', user);
				return fail(500);
	...
	editPersonalization: async (event) => {
		return await formRunner(['background', 'lang', 'daily'], async (
			...
			try {
				await event.locals.db.update(schema.user).set({
					...
					lang: formData['lang'],
					gamification: user.canEditGamification ? Boolean(formData['daily']) : user.gamification,
				}).where(eq(schema.user.uuid, user.uuid));
			} catch (e) {
				writeLog(event, 'ERROR', 'DB failure.', user);
				return fail(500);
