...
>
	<i
		class="ri-error-warning-line absolute top-1/6 right-1/10 scale-150 rotate-12 text-9xl text-white opacity-10"
	></i>
	<i
		class="ri-link-unlink absolute bottom-1/4 left-1/10 scale-150 -rotate-12 text-9xl text-white opacity-10"
	></i>

	<div
		class="z-10 flex w-full max-w-md flex-col items-center gap-8 rounded-3xl p-4 shadow-2xl backdrop-blur-sm sm:rounded-4xl sm:p-8 {darkMode
			? 'bg-neutral-800/80 text-white'
			: 'bg-white/90 text-black'}"
	>
		<header class="flex flex-col items-center gap-4 text-center">
			<div
				class="flex h-20 w-20 animate-pulse items-center justify-center rounded-3xl border-2"
		...
		<div class="w-full space-y-3">
			...
			<button
				...
				class="w-full text-sm font-semibold opacity-50 transition-opacity hover:opacity-100"
			>
				Go Back
			</button>
		</div>
