...
<div
	class="flex min-h-0 max-w-1/4 min-w-36 grow flex-col gap-2 overflow-auto rounded-e-2xl p-2 shadow-lg sm:min-w-40 {darkMode
		? 'bg-neutral-800/70'
		: 'bg-neutral-200/70'}"
...
>
	...
	>
		<div class="flex w-full flex-row items-start px-1">
			<i class="absolute top-0 right-0 {emoji} text-2xl md:text-3xl"></i>
			<h1 class="pe-8 text-xl font-bold sm:pt-0 sm:text-2xl">{title}</h1>
		</div>
