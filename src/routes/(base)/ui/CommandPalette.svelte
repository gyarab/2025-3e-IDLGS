...
{#if open}
	<div class="flex flex-col items-center justify-center fixed top-0 left-0 w-screen! h-screen! bg-neutral-900/50 z-50!">
		<dialog 
			bind:this={dialog}
			class="absolute top-1/3 left-1/2 -translate-1/2 z-51! w-1/2 h-1/2 flex flex-col grow gap-2 rounded-xl p-5 {darkMode ? 'bg-neutral-700/70 text-white' : 'bg-white/70 text-black'}"
