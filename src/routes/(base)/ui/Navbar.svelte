	...
	{#if !open}
		<i
			class="ri-arrow-down-s-fill {darkMode
				? 'text-neutral-700'
				: 'text-white'} absolute -top-6 left-3/4 text-6xl! opacity-100!"
...
{#if open}
	<div
		class="{darkMode
			...
			: 'bg-neutral-100 text-black'} fixed z-10! flex h-[7svh] w-screen grow flex-col items-center justify-center gap-2 border-b-2 shadow-xl"
		style="border-color: {accentColor};"
		transition:fly|global={{ y: -100, duration: 200, opacity: 0 }}
	>
		<div
			class="flex h-full w-2/3 flex-row items-center justify-center gap-4"
		>
			<div class="grow"></div>
