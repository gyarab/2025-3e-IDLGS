...
export const exercise = pgTable('exercise', {
	...
	*/
	...
	crosswordDataId: integer('crosswordDataId').references(() => exerciseCrossword.id),
	...
	check('type', sql`${table.type} IN ('CRS', 'CRW', 'DEF', 'GRP', 'GEO', 'EXT')`),
]);
...
export const exerciseCrossword = pgTable('exerciseCrossword', {
	...
	descriptions: text('descriptions').array().default(sql`ARRAY[]::text[]`),
	clue: text('clue').array().default(sql`ARRAY[]::text[]`),
});

export const exerciseCRW = pgTable('exerciseCRW', {
	id: serial('id').primaryKey().notNull().unique(),
	// starting positions and direction of words
	xStarts: integer('xStarts').array().default(sql`ARRAY[]::integer[]`).notNull(),
	yStarts: integer('yStarts').array().default(sql`ARRAY[]::integer[]`).notNull(),
	directions: text('directions').array().default(sql`ARRAY[]::text[]`).notNull(),
	// ids of letters in solution word
	xIds: integer('xIds').array().default(sql`ARRAY[]::integer[]`).notNull(),
	yIds: integer('yIds').array().default(sql`ARRAY[]::integer[]`).notNull(),
});

export const exerciseCRS = pgTable('exerciseCRS', {
	id: serial('id').primaryKey().notNull().unique(),
	offsets: integer('offsets').array().default(sql`ARRAY[]::integer[]`).notNull(),
