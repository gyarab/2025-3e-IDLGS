...
export const productKey = pgTable('productKey', {
	...
	key: text('key').notNull().$defaultFn(() => crypto.randomUUID().replaceAll('-', '').toLocaleUpperCase()),
...
export const purchase = pgTable('purchase', {
	...
	time: timestamp('time').notNull().$defaultFn(() => new Date()),
	});
...
export const school = pgTable('school', {
	...
	publicKey: text('publicKey').notNull().$defaultFn(() => crypto.randomUUID()),
});
