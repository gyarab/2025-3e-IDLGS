...
export const hashPassword = (password: string, amount?: number | undefined, salt?: string | undefined) => {
...
export const createUser = async (
	...
	return (await db.insert(dataSchema.user).values({
		email: email,
...
export const getUser = async (event: RequestEvent): Promise<UserType | undefined> => {
	...
	const token = await db.select().from(dataSchema.userSession).where(eq(
		dataSchema.userSession.token,
		cookie
	)).limit(1);
	if(token.length == 0) return undefined;

	if(token[0].expiresAt <= new Date()) {
		await db.delete(dataSchema.userSession).where(eq(
			dataSchema.userSession.token,
			cookie
		));
		return undefined;
